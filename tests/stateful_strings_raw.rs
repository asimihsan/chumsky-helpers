use chumsky::Parser;
use chumsky_helpers::stateful_strings::{cooked_string, interpolated_string, raw_string, Segment};

#[test]
fn raw_single_line_zero_hash() {
    let out = raw_string::<false>().parse("\"hello\"").into_result().unwrap();
    assert_eq!(out, "hello");
}

#[test]
fn raw_single_line_three_hashes() {
    let out = raw_string::<false>().parse("###\"abc\"###").into_result().unwrap();
    assert_eq!(out, "abc");
}

#[test]
fn raw_missing_closing_delimiter_is_error() {
    assert!(raw_string::<false>().parse("##\"abc").has_errors());
}

#[test]
fn raw_multi_line_indent_preserved_for_now() {
    let src = "#\"\"\"\n    foo\n    bar\n    \"\"\"#";
    let out = raw_string::<true>().parse(src).into_result().unwrap();
